...
describe("UCSBOrganizationIndexPage tests", () => {
  ...
  test("renders three ucsborganizations correctly for regular user", async () => {
    ...
    await waitFor(() => {
      ...
      ).toHaveTextContent("MHJ");
    });
    expect(
      screen.getByTestId(`${testId}-cell-row-1-col-orgCode`),
    ).toHaveTextContent("CHS");
    expect(
      screen.getByTestId(`${testId}-cell-row-2-col-orgCode`),
    ).toHaveTextContent("MAG");
    ...
    const orgTranslationShort = screen.getByText("Mahjong Club");
    expect(orgTranslationShort).toBeInTheDocument();

    const orgTranslation = screen.getByText("Asian Board Games Club");
  ...
  test("what happens when you click delete, admin", async () => {
    ...
    axiosMock
      ...
      .reply(200, "UCSBOrganization with orgCode MHJ was deleted");
    ...
    await waitFor(() => {
      expect(mockToast).toBeCalledWith(
        "UCSBOrganization with orgCode MHJ was deleted",
      );
    });
    ...
    expect(axiosMock.history.delete[0].url).toBe("/api/ucsborganization");
    expect(axiosMock.history.delete[0].params).toEqual({ orgCode: "MHJ" });
  });
});